...
resources, typed request context facades, order preview caching, and Discord/Signal
...
| `src/schwab_mcp/approvals/` | Approval abstraction with Discord- and Signal-backed approval workflows for Schwab write tools. | [View Map](src/schwab_mcp/approvals/codemap.md) |
...
- Approval integration uses a process-local `ApprovalManager` interface with a
  Discord and Signal implementations for human decisions and a no-op implementation for
