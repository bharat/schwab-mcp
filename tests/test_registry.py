...
# ---------------------------------------------------------------------------
# _format_argument rendering (fork behavior: full JSON, never truncated, so
# reviewers always approve the exact arguments; oversized requests are
# auto-denied by the approval backends instead)
# ---------------------------------------------------------------------------


def test_format_argument_never_truncates_long_values() -> None:
    long_str = "x" * 300
    result = _format_argument(long_str)
    assert result == json.dumps(long_str)
    assert not result.endswith("...")


def test_format_argument_short_values_json_encoded() -> None:
