...
def test_server_exits_when_easy_client_raises(monkeypatch):
    ...
    assert result.exit_code == 1
    # Fork hardening: the raw exception text is never leaked into the MCP
    # error response; only the exception type is surfaced.
    assert "Failed to initialize Schwab client; see server logs." in result.output
    assert "RuntimeError" in result.output
    assert "auth exploded" not in result.output
...
def test_server_warns_when_jesus_flag_and_discord_token_both_set(monkeypatch):
    ...
    assert result.exit_code == 0
    # Warning goes to stderr. The fork warns unconditionally (not just when a
    # Discord token is configured) since the flag disables ALL human approval.
    assert "--jesus-take-the-wheel is set" in (result.output + (result.stderr or ""))
    assert "NO human approval" in (result.output + (result.stderr or ""))
...
def test_server_exits_when_server_run_raises(monkeypatch):
    ...
    # Fork hardening: the raw exception text is never leaked into the MCP
    # error response; only the exception type is surfaced.
    assert "Server failed; see server logs." in result.output
    assert "RuntimeError" in result.output
    assert "server exploded during run" not in result.output
