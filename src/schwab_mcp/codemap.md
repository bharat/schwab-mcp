...
- `server.py`: MCPServer adapter. `SchwabMCPServer` constructs `MCPServer`, installs `_client_lifespan()`, registers tools/resources, and chooses a result transform: Toon-encoded stripped payloads by default or stripped JSON when `use_json=True`. `send_error_response()` emits JSON-RPC 2.0 errors to stdout before the MCP server is initialized.
- `context.py`: typed dependency container. `SchwabServerContext` stores the raw `AsyncClient`, `ApprovalManager`, `PreviewStore`, and typed client facades cast from `schwab_mcp.tools._protocols`. `SchwabContext` subclasses MCPServer `Context` and exposes safe properties for tools.
...
4. The server command selects write permissions: `--jesus-take-the-wheel` uses `NoOpApprovalManager` and enables writes; Discord configuration creates `DiscordApprovalManager`; Signal configuration creates `SignalApprovalManager`; otherwise writes are disabled and a no-op manager is still used for lifecycle consistency.
5. `SchwabMCPServer.__init__()` creates `MCPServer` with `_client_lifespan()`, registers all tools via `tools.register_tools()` with write/technical/result-transform flags, then registers resources via `resources.register_resources()`.
6. On MCPServer startup, `_client_lifespan()` starts the approval manager and yields `SchwabServerContext`. Tool registration wrappers in `schwab_mcp.tools._registration` convert generic MCP contexts to `SchwabContext`, apply approval gating for write tools, and apply the configured result transform.
...
- Entry points: `schwab_mcp.main()`, Click commands in `cli.py`, and `SchwabMCPServer.run()` for MCPServer stdio transport.
...
- MCP integration: `server.py` uses `mcp.server.mcpserver.MCPServer`; `context.py` subclasses MCPServer `Context`; `resources.py` registers URI-addressable resources; `tools.register_tools()` registers callable Schwab operations.
- Approval integration: `cli.py` constructs `NoOpApprovalManager`, `DiscordApprovalManager`, or `SignalApprovalManager`; `server._client_lifespan()` owns start/stop; `tools._registration.register_tool()` wraps write tools with approval requests and progress reporting.
