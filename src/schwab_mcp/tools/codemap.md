...
actions through MCPServer tools. Modules keep Schwab client calls small and typed,
...
- `_registration.py` centralizes MCPServer registration. `register_tool()` converts
...
- Order construction is split between `orders.py` orchestration and
  ...
  trigger, bracket, and option-combo specs. Prices are passed to schwab-py as
  truncated strings (`_price_str`) to match Schwab's wire format exactly.
- `orders.py` also applies an upfront assetType guard: EQUITY-leg symbols are
  resolved via the instruments endpoint at preview time (and defensively at
  `place_previewed_order`), rejecting unsupported types such as mutual funds
  and fixed income with `UnsupportedAssetTypeError` while allowing ETFs.
  Lookup failures degrade gracefully and defer validation to the Schwab API.
- `fishing.py` runs range-based, chunked option-order fishing campaigns: one
  approved submission spawns a background task that places LIMIT chunks and
  cancel/replaces them toward a terminal price. It calls `orders.py` internal
  builders directly and tracks campaigns in an in-memory registry.
...
3. At invocation, MCPServer passes an MCP context. Registration wrappers convert it
...
- Uses MCPServer's `server.tool()` through `_registration.register_tool()` so tool
